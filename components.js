angular.module('ny-components', [])
.directive('nyImg', function(){
    return {
      restrict: 'E',
      replace: true,
      ...
      require: '^nyLazyScrollable',
      link: function(scope, element, attrs, ctrl){
        ctrl._addLazyItem(scope, element);
      },
...
.directive('nyLazyScrollable', function(){
  return {
    restrict: 'A',
    controller: ['$scope', '$element', '$attrs', function($scope, $element, $attrs){
      var lazyItems = [];
      this._addLazyItem = function(_scope, _ele){
        lazyItems.push({
          scope: _scope,
          element: _ele
        });
      ...
      $element.on('scroll', function(){
        var bottom = verge.rectangle($element).bottom;
        for(var i = 0, n = lazyItems.length; i < n; i++){
          var top = verge.rectangle(lazyItems[i].element).top;
          if(bottom >= top){
            lazyItems[i].scope._lazyLoad();
            //remove item showed
